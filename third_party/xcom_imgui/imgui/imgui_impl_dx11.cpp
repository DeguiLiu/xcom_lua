...
static HMODULE g_imgui_d3dcompiler = nullptr;

static HRESULT ImGui_ImplDX11_CompileShader(const void* source, SIZE_T source_size,
                                            ...
                                            ID3DBlob** blob)
{
    using CompileFn = HRESULT (WINAPI*)(LPCVOID, SIZE_T, LPCSTR,
                                        ...
                                        ID3DBlob**, ID3DBlob**);
    constexpr LPCWSTR compiler_names[] = {
        L"D3DCOMPILER_47.dll", L"D3DCOMPILER_43.dll",
    };
    if (g_imgui_d3dcompiler == nullptr) {
        for (const LPCWSTR name : compiler_names) {
            g_imgui_d3dcompiler = LoadLibraryW(name);
            if (g_imgui_d3dcompiler != nullptr) break;
        }
    }
    if (g_imgui_d3dcompiler == nullptr) return E_NOINTERFACE;
    const auto compile = reinterpret_cast<CompileFn>(
        GetProcAddress(g_imgui_d3dcompiler, "D3DCompile"));
    return compile == nullptr
        ? E_NOINTERFACE
        : compile(source, source_size, nullptr, nullptr, nullptr, entry_point,
                  target, 0, 0, blob, nullptr);
}

static void ImGui_ImplDX11_UnloadCompiler() noexcept
{
    if (g_imgui_d3dcompiler != nullptr) {
        FreeLibrary(g_imgui_d3dcompiler);
        g_imgui_d3dcompiler = nullptr;
    }
}

bool    ImGui_ImplDX11_CreateDeviceObjects()
{
    struct CompilerScope final {
        ~CompilerScope() { ImGui_ImplDX11_UnloadCompiler(); }
    } compiler_scope;
    ImGui_ImplDX11_Data* bd = ImGui_ImplDX11_GetBackendData();
    if (!bd->pd3dDevice)
        return false;
    ...
    // Create the vertex shader
    {
        ...
        if (FAILED(ImGui_ImplDX11_CompileShader(vertexShader, strlen(vertexShader), "main", "vs_4_0", &vertexShaderBlob)))
    ...
    // Create the pixel shader
    {
        ...
        if (FAILED(ImGui_ImplDX11_CompileShader(pixelShader, strlen(pixelShader), "main", "ps_4_0", &pixelShaderBlob)))
            return false; // NB: Pass ID3DBlob* pErrorBlob to D3DCompile() to get error showing in (const char*)pErrorBlob->GetBufferPointer(). Make sure to Release() the blob!

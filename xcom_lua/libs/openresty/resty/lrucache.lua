...
--
-- Patched for xcom_lua by removing the `ngx.now` / `ngx.log` dependencies so
-- this LRU cache works in a standalone LuaJIT process.
-- Upstream: openresty-1.29.2.1-win64/lualib/resty/lrucache.lua  (v0.14)
--
-- Changes vs upstream:
--   * `local ngx_now = ngx.now`  ->  `local ngx_now` declared as upvalue
--     and resolved lazily via `lrucache._set_now_fn(fn)`. Default fallback is
--     `os.time` (seconds since epoch, integer). Inject e.g.
--     `require("ffi").cast("double", require("ffi").C.clock())` for ms precision.
--   * The LuaJIT 2.0 warning at module load (which used `ngx.log`) is wrapped
--     in pcall and silently dropped — `ngx` is undefined in this host.
--   * Two call sites of `ngx_now()` (lines 198 / 266 upstream) keep working
--     unchanged because they read the upvalue.
...
-- LuaJIT 2.0 advisory — wrapped in pcall because `ngx` is undefined here.
do
    local ok = pcall(function()
        if string.find(jit.version, " 2.0", 1, true) then
            ...
            -- Patched: silent no-op in non-nginx host. Users who want the
            -- warning can wire their own sink via _set_now_fn comment below.
        end
    end)
end
...
-- ----- xcom_lua patch: injectable clock --------------------------------------
-- ngx.now() replacement. Upvalue, set lazily via _set_now_fn().
-- Default: os.time (integer seconds). Swap to monotonic ms if needed:
--     lrucache._set_now_fn(function() return os.time() end)               -- default
--     lrucache._set_now_fn(function()
--         local ffi = require("ffi"); ffi.cdef[[ long long msec(); ]]
--         return ffi.C.msec() / 1000.0
--     end)
local ngx_now = os.time

function _M._set_now_fn(fn)
    assert(type(fn) == "function", "now_fn must be a function returning seconds")
    ngx_now = fn
end
------------------------------------------------------------------------------
